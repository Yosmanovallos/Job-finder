...
| 2026-09-15 | P3 | Implementada, **sin desplegar**. `FetchContext` opcional (deadline + `AbortSignal` + presupuesto) cableado solo en los puntos compartidos — adaptadores sin tocar, heredan la cancelación en la frontera del wrapper; instrumentación por adaptador queda para P4/P5. Presupuesto **derivado** del plazo global con reserva de cierre fuera del reparto (`tick-budget.ts`), cierre en 5 pasos todos acotados (`pool.end()` con tope y salida explícita), `scrape_leases` aditiva con reclamación atómica en una sentencia + latido reutilizando el de P2. Tabla propia y no columnas en `role_source_runs` porque `markRoleForImmediateRescan` borra las filas de ese rol y se llevaría el lease por delante (EXE-008 lo prueba). Roles vencidos ya informan lo completado (EXE-009). `npm ci` sin fallback y cron a `*/30` (experimento con revisión el 2026-09-22). **Node 24 separado a un segundo despliegue** por decisión del usuario: es el único cambio con riesgo real para producción (comportamiento de red de got-scraping/playwright/undici contra fuentes reales, no verificable en local) y merece su propio canario, así que los workflows siguen en Node 20 y el EBADENGINE persiste hasta entonces. EXE-001…010 trazados. Gates: unit 12/12 + 11/11, integración completa (OBS-001…012 + EXE), baseline y build ✅, tsc **29** / eslint **295** = línea base heredada, **0 nuevos**. Migración aplicada solo a base desechable; la real sigue sin migrar. Pendiente arrastrado: `OPS_ADMIN_TOKEN` en `.env.example` (ruta denegada a agentes, la añade el usuario). |
