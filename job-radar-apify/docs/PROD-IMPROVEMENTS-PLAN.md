...
**Desplegado:** `52e1898` (P0-P3) — `origin/main`, 2026-09-16
...
| P4 | Contrato `SourceFetchResult`, transporte/proxy por política | ✅ Done | `634a358` | `openspec/changes/archive/p4-source-contract/` |
| P5 | Recuperación de adaptadores (una fuente por entrega) | 🔵 Spec draft | — | `openspec/changes/p5-adapter-recovery/` |
...
| 2026-09-16 | P3 (corrección) | Commits `88bce64`, `de305c8`, `52e1898`. **P3 necesitó tres despliegues.** (1) Plazos. (2) Node 24: los 6 avisos `EBADENGINE` de `@supabase/*` desaparecieron, pero quedaba uno autoinfligido — `engines.node` fija la versión **exacta** `24.18.0` y pedir `"24"` trae 24.20.0 — resuelto fijando `24.18.0` en los 3 workflows; `EBADENGINE` = 0. Browser-tick verificado a mano (Playwright + proxy residencial intactos: Glassdoor-CO 158, Glassdoor-VE 26, Indeed-CO 51, Indeed-VE 12; el proxy se recuperó solo de un 403 rotando IP). (3) **Regresión introducida por P3 y detectada por su propio canario** (run `35049467975`, `cancelled` a 27m21s): al sustituir el `Promise.race` por un `await` simple, la espera del rol quedó SIN límite — `hasBudget` comprueba antes de arrancar, pero `adapter.fetch()` recorre sus ~15-20 variantes de keyword por dentro y no recibe el signal, así que a los 27 min Magneto seguía buscando y el cierre nunca se ejecutó. Corregido con `whenAborted(ctx)` + `Promise.race` (abandonar es seguro ahora y no lo era antes de P3: guardado por adaptador, `perSource` conservado, gracia acotada y salida explícita) y un vigilante global. **Además la estimación de 30 s por fuente era errónea por hasta 10x**: medido sobre `source_attempts` (p50) Elempleo 118 s, LinkedIn 81 s, Computrabajo 75 s, Magneto 53 s, Torre 26 s → `SOURCE_LISTING_ESTIMATE_MS` por fuente, con **p50 y no p95 a propósito** (el p95 de Elempleo, 278 s, supera el techo de un lote y la dejaría en inanición permanente). Canario final `35051744585`: `success`, **13m01s**, cierre ordenado ejecutado (`Hasta 60s para escrituras` → `📡 Ejecución partial` → `✅ Finalizado en 783s`), `Timeouts: 1` con el tick siguiendo adelante, 0/6 ejecuciones por plazo vencido, 0 por encima de 20 min. Lección registrada: una estimación inventada es deuda hasta que se mide. |
| 2026-09-16 | P3 | Commit `58098d8`, **desplegado**. `FetchContext` opcional (deadline + `AbortSignal` + presupuesto) cableado solo en los puntos compartidos — adaptadores sin tocar, heredan la cancelación en la frontera del wrapper; su instrumentación interna queda para P4/P5. Presupuesto **derivado** del plazo global con reserva de cierre fuera del reparto (`tick-budget.ts`), y cierre en 5 pasos todos acotados, incluido `pool.end()` con tope de 10 s y salida explícita — ese `pool.end()` sin límite era el tramo que llegaba a 27-28 min y se llevaba el hard-kill. `scrape_leases` aditiva con reclamación atómica en una sentencia (`WHERE` sobre `DO UPDATE`) y latido reutilizando el de P2; tabla propia y no columnas en `role_source_runs` porque `markRoleForImmediateRescan` borra las filas de ese rol y se llevaría un lease vivo por delante (EXE-008 lo prueba). Reclamación **perezosa**: se reclama cuando el rol arranca, porque reclamar los 8 candidatos por adelantado dejaba bloqueados ~10 min los que el presupuesto no alcanzaba si el proceso moría — peor que no reclamar. Roles vencidos ya informan lo completado (EXE-009). `npm ci` sin fallback y cron a `*/30` (experimento, revisión 2026-09-22, ADR `docs/adr/0003-scheduler-cadence.md`). EXE-001…010 trazados. Gates: unit 23/23, integración completa (OBS-001…012 + EXE-003/005/006/007/008/009), baseline y build ✅, tsc **29** / eslint **295** = línea base heredada, **0 nuevos**. Canario `35046815422`: `success`, **18m34s**, cero regresiones por fuente (Computrabajo 8→22, Elempleo 12→32, LinkedIn 79→139, Torre 61→72, WorkanaV2 ausente→57; ninguna pasa de `success` a `blocked`/`empty`). **Matiz:** el paso de `timeout` a `partial` no es atribuible solo a P3 — una ejecución pre-P3 (00:47) también salió `partial`, así que las 23:28 era el peor caso, no el invariable; confirmarlo requiere varios días de `scrape_runs`. **Node 24 separado a un segundo despliegue** por decisión del usuario (único cambio con riesgo real: comportamiento de red de got-scraping/playwright/undici contra fuentes reales, no verificable en local): los workflows siguen en Node 20 y el EBADENGINE persiste. `scrape-browser-tick.yml` sin verificar (corre cada 2 días). **`scrape_leases` migrada** en la base real el 2026-09-16 (corrida dos veces, idempotente; verificado: RLS activo, 0 grants a `anon`/`authenticated`, 0 leases vivos, corpus intacto — 67.028 vacantes / 53 usuarios / 6 transacciones / 35 roles antes y después). Pendiente arrastrado: `OPS_ADMIN_TOKEN` en `.env.example` de `main`. |
...
| 2026-09-17 | P4 | Commit `634a358`, **desplegado** (fast-forward `6ba16ea..634a358`, sin forzar). `SourceFetchResult` tipado con el vocabulario que `source_attempts` ya admitía desde P2 y que **nadie emitía** (0 apariciones de `blocked`/`rate_limited`/`quota_exhausted`/`misconfigured`/`schema_changed` en 7 días: el contrato `Promise<Job[]>` no podía decirlo). **Hallazgo principal, medido antes de diseñar (SRC-003): el circuito de detalle no podía abrirse.** `scrape-worker` convertía `fetchDetail() === null` en `[]` y `executeWithResilience` trataba cualquier array como éxito, así que cada detalle nulo llamaba a `recordSuccess` y borraba el contador. Prueba en la tabla real: `source_circuit_state` tenía 4 filas y **ninguna `-detail`** (`recordFailure` hace INSERT, `recordSuccess` solo UPDATE → la ausencia de la fila demuestra que nunca se llamó), mientras Computrabajo gastaba **102 páginas de detalle para 0 resultados**. Arreglado con un tercer efecto de circuito: `empty` es **neutro** (ni reinicia ni incrementa). **Dos premisas del borrador cayeron con los datos:** (1) no hay inanición de presupuesto — **0 listados omitidos en 7 días**, así que el orden de adopción sale del fallo concentrado en detalle, no del presupuesto; (2) los circuitos de listado y detalle ya eran filas distintas — lo compartido era la *política*, ahora resoluble por (fuente, etapa). `BUDGET_ESTIMATES.detailFetch` **se queda en 8000 ms, ahora medido**: p50 3454 / p95 5816 / **max 7312** ms por página; una lectura previa basada en la **media** (3,1-4,0 s) sugería bajarlo y era el estadístico equivocado para una decisión de presupuesto — bajarlo habría creado inanición en el detalle. Adopción gradual con `fetchResult?()` como método **opcional** de `SourceAdapter`, no unión en el retorno de `fetch`: una unión habría obligado a discriminar a los 16 sin migrar y roto el gate de 0 errores nuevos. Solo Torre migra. **Tres límites documentados, no ocultados:** (a) **SRC-008 sigue siendo heurístico para el rechazo por validación** — `liftJobArray` fija `valid = received` porque la validación ocurre después en `saveJobs`, así que la rama `all_rejected_by_validation` del contrato no es alcanzable para un adaptador sin migrar y Jooble lo sigue clasificando la contabilidad de P2; (b) **Torre demuestra el mecanismo, no clasificación fina** — `scrapeTorre` se traga una respuesta no-ok y cualquier excepción en `[]`, y distinguirlo es reparación de adaptador, o sea P5; (c) **el circuito `-detail` se cierra por spec y prueba de integración, no por canario** — en los 3 ticks no hubo ni un fallo real de detalle (solo `empty`/`partial`/`success`), así que la fila sigue legítimamente ausente y la condición de refutación (≥1 fallo real y ninguna fila) no se dio. Gates: unit **18/18** nuevas (P3 15/15 intactas), integración **7/7** nuevas (OBS-001…012 y EXE-003…009 intactas), baseline 9 rutas, build ✅, `tsc` **29** = línea base heredada (**0 nuevos**), `eslint` **294** frente a 295 heredados (**0 nuevos, 1 preexistente eliminado** al reescribir `resilient-fetch`; verificado linteando las versiones de `HEAD` junto a las nuevas, porque un total que baja también hay que explicarlo). **Tres canarios leídos por log, no por exit code:** `35171209713` (812 s) y `35173002214` (793 s) sobre la rama **antes** del merge, y `35175143321` (818 s) post-deploy desde `main`; los tres con cierre ordenado, `Timeouts: 1`, **0** menciones de Circuit Breaker y **0** fallos de detalle. `partial/some_sources_degraded` **no es regresión de P4**: las dos ejecuciones previas de `main` sin P4 salieron igual. Ninguna fuente pasó de `success` a `blocked`/`empty` y `valid == received` en todos los listados (Torre 291/291, 224/224, y su agregado sigue `success` pese a varios `Found 0 jobs` por palabra clave). La mejora sí se ve en la clasificación: Computrabajo detalle pasa de `empty` a secas (21/0 pre-P4) a `empty,partial` (33/1 y 70/1). Sin cambios de esquema, así que el rollback es revertir el commit. **Cobertura declarada honestamente: 5-6 de 17 fuentes observadas; Glassdoor-CO/VE e Indeed-CO/VE siguen SIN VERIFICAR** (viven en `scrape-browser-tick.yml`, cada 2 días) — pendiente observado de P4, no cubierto. `/api/runs` responde el contrato nuevo y `/api/admin/runs` sigue dando 401 sin token. |
