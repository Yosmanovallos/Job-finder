...
export const TRANSLATION_MAP: Record<string, string[]> = {
  ...
  enfermero: ["nurse"],
  enfermera: ["nurse"],
  enfermería: ["nurse", "nursing"],
  enfermeria: ["nurse", "nursing"],
  auxiliar: ["assistant", "aide"],
  ejecutivo: ["executive"],
  ejecutiva: ["executive"],
  executive: ["ejecutivo", "ejecutiva"],
  // "admin" deliberately excluded — as a short substring it false-matches
  // "administrador"/"administrator" (different word, different role), unlike
  // the full word "administrative" which doesn't share that prefix collision.
  ...
  calidad: ["quality", "qa"],
  quality: ["calidad"],
  // "qa" itself had no reverse entry, so searching/filtering for "QA
  // Engineer" never expanded to "Ingeniero de Calidad" — the exact gap that
  // caused fewer real postings to be found for that role.
  qa: ["calidad", "quality"],
...
export function generateRoleKeywordsWithAI(requestedRoles: string[]): string[] {
  ...
  for (const rawRole of requestedRoles) {
    ...
    const words = lower
      .split(/\s+/)
      .filter(
        (w) => w.length > 1 && w !== "de" && w !== "del" && w !== "en" && w !== "y" && w !== "o"
      );
    ...
    if (
      ...
      lower.includes("gerente de proyecto") ||
      lower.includes("lider de proyecto")
    ) {
      ...
      lower.includes("cuidador") ||
      lower.includes("cuidadora") ||
      lower.includes("abuelo") ||
      lower.includes("abuela") ||
      lower.includes("geriatr") ||
      lower.includes("adulto mayor")
    ) {
