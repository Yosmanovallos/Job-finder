...
export class ScrapeWorker {
  ...
  private checkMemory() {
    ...
    if (heapUsedMb > 200) {
      console.warn(`⚠️ [ScrapeWorker] Consumo de memoria RAM elevado (${heapUsedMb.toFixed(2)} MB > 200 MB). Ejecutando limpieza...`);
      if (global.gc) {
   ...
   */
  async processRoleJob(options: WorkerJobOptions): Promise<{ roleName: string; totalJobs: number; savedCount: number; duplicateCount: number }> {
    this.checkMemory();

    const { roleName, dateRange = '48h' } = options;
    ...
    for (const adapter of allAdapters) {
      try {
        ...
        console.error(`❌ [ScrapeWorker] Error en adaptador ${adapter.name} procesando "${roleName}":`, err?.message || err);
      }
    }
    ...
    console.log(`✅ [ScrapeWorker] Rol "${roleName}" completado: ${accumulatedJobs.length} vacantes encontradas (${savedCount} nuevas, ${duplicateCount} duplicadas fusionadas).`);
